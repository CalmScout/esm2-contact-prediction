...
Examples:
  ...
  %(prog)s --dataset-path data/cnn_dataset.h5 --base-channels 64 --mlflow-experiment large_model_experiment
  %(prog)s --dataset-path data/cnn_dataset.h5 --train-ratio 0.9 --val-ratio 0.05 --test-ratio 0.05
    ...
    # Note: Experiments are now tracked exclusively in MLflow
    ...
    # MLflow arguments (MLflow is now mandatory)
    parser.add_argument(
        '--mlflow-experiment',
        type=str,
    ...
    # Note: All outputs are now logged to MLflow, no local directory creation
...
# Note: Experiments are now tracked exclusively in MLflow, no local directory creation needed
...
def create_training_config(args: argparse.Namespace, dataset_path: str, test_batch_size: int) -> Dict[str, Any]:
    ...
    config = {
        ...
        # Experiment configuration (MLflow-only)
...
def log_results_to_mlflow(history: Dict[str, np.ndarray],
                         best_auc: float, test_results: Dict[str, float],
                         model: torch.nn.Module, config: Dict[str, Any],
                         training_time: float) -> None:
    """Log all experiment results and artifacts to MLflow (MLflow-only approach)."""
    import mlflow
    import tempfile
    import os
...
def main():
    ...
    # Setup MLflow (mandatory)
    print("🔄 Setting up MLflow (mandatory for all experiments)...")
    ...
    # Create training configuration (MLflow-only)
    config = create_training_config(args, args.dataset_path, test_batch_size)

    # Create trainer and start training (MLflow is now mandatory)
    ...
    trainer = CNNTrainer(
        ...
        enable_mlflow=True,  # MLflow is now mandatory
        mlflow_experiment=args.mlflow_experiment
    )
